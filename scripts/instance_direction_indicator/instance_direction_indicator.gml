...
function instance_direction_indicator(_object_type, _color){
	...
	for(var i=0; i<_object_type_number; i++){
		...
		for(var j=0; j<_ins_n; j++){
			...
			if(_object_type[i] = o_enemy){
				...
				and _ins_id.soul_enemy){
					//一定距離内にいる画面に写ってない敵をds_indicatorにいれる soul_enemy以外は見えない
					...
					ds_grid_set(ds_out_of_screen_enemy, DirectionIndicator.FarDetect, _detect_number, false);
				...
				and !_ins_id.hidden_goal){
					...
					ds_grid_set(ds_out_of_screen_enemy, DirectionIndicator.Distance, _detect_number, _distance_for_instance);
					ds_grid_set(ds_out_of_screen_enemy, DirectionIndicator.Direction, _detect_number, _direction_for_instance);
					ds_grid_set(ds_out_of_screen_enemy, DirectionIndicator.FarDetect, _detect_number, true);
	...
	for(var i=0; i<_ds_height; i++){
		...
		var _id = ds_grid_get(ds_direction_arrow, DirectionIndicator.Id, i);
		...
		//消える
			...
			ds_grid_set(ds_direction_arrow, DirectionIndicator.FarDetect, i, noone);
	...
	for(var i=0; i<_detect_max; i++){
		...
		var _detect_far_detect = ds_grid_get(ds_out_of_screen_enemy, DirectionIndicator.FarDetect, i);
		var _finded = false;
		...
		for(var j=0; j<_ds_height; j++){
			if(_detect_id == ds_grid_get(ds_direction_arrow, DirectionIndicator.Id, j)){
				...
				ds_grid_set(ds_direction_arrow, DirectionIndicator.FarDetect, j, _detect_far_detect);
