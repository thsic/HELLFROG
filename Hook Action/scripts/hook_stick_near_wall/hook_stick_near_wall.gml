...
//壁への角度を求める
	...
	//ここらへんのcollision_size*4は、射程内にo_wallが存在しないのにcollision_length_checkを実行するとばぐりそうがからあえて大きくしてる
	...
	if(collision_line(hook_point_x, hook_point_y, _temp_x, _temp_y, o_wall, false, true)){
		//90度刻みで壁にぶつかった場合
		var _distance_for_wall = point_distance(hook_point_x, hook_point_y, _temp_x, _temp_y);
		...
		debug_draw_line(hook_point_x, hook_point_y, hook_point_x+lengthdir_x(32, _direction_for_wall_corner,), hook_point_y+lengthdir_y(32, _direction_for_wall_corner), c_white, 60)
		...
		var _width_sign = sign(lengthdir_x(_direction_for_wall_corner+180, 1));
		var _height_sign = sign(lengthdir_y(_direction_for_wall_corner+180, 1));
		...
		debug_wall_corner_point_x = _wall_corner_point_x;
		debug_wall_corner_point_y = _wall_corner_point_x;
